...
def build_filename_mask(slide_name, tile_size, poi, level):
    """Generate hdfs filename.
...
def generate_positive_slides(mgr, level, tile_size, poi_tumor, percent_overlap, max_tiles_per_slide,
    ...
    for i in range(num_slides):
        ...
        try:
            ...
            store_slides_hdfs(filename_mask, slide.name, num_tiles_batch, masks_batch, tile_size)
...
def main():
    global LOGGER, HDFS_DIR, MASK_DIR
    ...
    parser.add_argument('--masks_folder', '-mf', dest='masks_folder', action='store',
                        default='masks_CAMELYON16', type=str,
                        help='tiles folder to store hfds files')
    ...
    MASK_DIR = os.path.join(cam_base_dir, args.masks_folder)
